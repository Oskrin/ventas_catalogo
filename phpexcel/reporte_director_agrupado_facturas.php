...
while ($row = pg_fetch_row($consulta)) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue("E6", 'DIRECTOR: ' . $row[2]);
    $objPHPExcel->setActiveSheetIndex(0)
            ->mergeCells('E6:F6');
    ...
    $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue("B7", 'TELF: ' . $row[4]. '-'. $row[5]);
    $objPHPExcel->setActiveSheetIndex(0)
            ->mergeCells('B7:C7');
    ...
    $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue("E7", 'DIRECCIÓN: ' . $row[3]);
    $objPHPExcel->setActiveSheetIndex(0)
            ->mergeCells('E7:F7');
        ...
        if(pg_num_rows($sql2)){
            ...
            while ($row2 = pg_fetch_row($sql2)) {            
                ...
                if($cont_sql2 == 0){                    
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle('C'.$yyy.':D'.$yyy)->getFont()->setBold(false)->setName('Verdana')->setSize(10);                      
